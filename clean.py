...
def clean_words(words):
    cleaned_words = []
    for word in words:
        cleaned_word = re.sub(r'\|.*$', '', word)  # Remove extra words after "|"
        if not cleaned_word.startswith('File:'):  # Exclude file lines
...
with open(input_file, 'r') as file:
    ...
    for line in file:
        ...
        if len(chunk) >= chunk_size:
            ...
            save_to_text_file(cleaned_words, 'cleaned_words.txt')
            chunk = []
